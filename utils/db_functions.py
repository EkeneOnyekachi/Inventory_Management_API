...
async def db_get_user_items(owner):
    query = """select name, quantity, price, description from items where owner = :owner"""
    values = {"owner":owner}
    result = await fetch(query, False, values)
    return result
...
async def db_get_item_with_id(id):
    query = """select name, quantity, price, description, owner from items where id = :id"""
    values = {"id":id}
...
async def db_update_item(id, name, quantity, price, description):
    ...
    values = {"name": name, "quantity": quantity, "price": price,
